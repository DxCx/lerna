...
var BootstrapCommand = function (_Command) {
  ...
  _createClass(BootstrapCommand, [{
    ...
    value: function bootstrapPackages(callback) {
      ...
      function (cb) {
        return _this3.symlinkDevBinaries(cb);
      },
    ...
    value: function prepublishPackages(callback) {
      ...
      var bootstrapBatch = function bootstrapBatch() {
        ...
        if (todoPackages.length && !batch.length) {
          _this4.logger.warning("Encountered a cycle in the dependency graph.  " + "This may cause instability if dependencies are used during `prepublish`.");
    ...
    key: "symlinkDevBinaries",
    value: function symlinkDevBinaries(callback) {
      var _this7 = this;

      _async2.default.waterfall([function (cb) {
        _this7.logger.info("Reading devDependencies");
        var devDependencyKeys = Object.keys(_this7.repository.packageJson.devDependencies || {});
        _this7.progressBar.init(devDependencyKeys.length);
        var actions = [];
        var rootBinaryDependencies = [];
        devDependencyKeys.forEach(function (dependency) {
          actions.push(function (cb) {
            var dependencyLocation = _path2.default.resolve(_this7.repository.rootPath, "node_modules", dependency);
            var dependencyPackageJsonLocation = _path2.default.join(dependencyLocation, "package.json");
            // ignore dependencies without a package.json file
            if (!_FileSystemUtilities2.default.existsSync(dependencyPackageJsonLocation)) {
              _this7.logger.error("Unable to find package.json for " + dependency + " dependency of " + (_this7.repository.packageJson.name || "root package") + ",  " + "Skipping...");
            } else {
              var dependencyPackageJson = require(dependencyPackageJsonLocation);
              if (dependencyPackageJson.bin) {
                rootBinaryDependencies.push({ dependency: dependency, dependencyLocation: dependencyLocation, bin: dependencyPackageJson.bin });
              }
            }
            _this7.progressBar.tick(dependency);
            cb();
          });
        });
        _async2.default.series(actions, function (err) {
      ...
      }, function (rootBinaryDependencies, cb) {
        _this7.logger.info("Symlinking devDependency binaries");
        _this7.progressBar.init(_this7.filteredPackages.length);

        var actions = [];
        _this7.filteredPackages.forEach(function (filteredPackage) {
          // actions to run for this package
          var packageActions = [];
          rootBinaryDependencies.forEach(function (_ref2) {
            var dependency = _ref2.dependency;
            var dependencyLocation = _ref2.dependencyLocation;
            var bin = _ref2.bin;

            var destFolder = filteredPackage.nodeModulesLocation;
            packageActions.push(function (cb) {
              _this7.createBinaryLink(dependencyLocation, destFolder, dependency, bin, cb);
            });
          });
          actions.push(function (cb) {
            _async2.default.series(packageActions, function (err) {
